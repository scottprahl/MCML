...
double PathToBoundary(PhotonStru *Photon_Ptr, InStru *In_Ptr)
{
    ...
    if (uz > 0.0)
        path = (In_Ptr->layerspecs[layer].z1 - Photon_Ptr->z) / uz;     /* path>0. */
    else if (uz < 0.0)
        path = (In_Ptr->layerspecs[layer].z0 - Photon_Ptr->z) / uz;     /* path>0. */
    else
 ...
 ****/
double RFresnel(double ni,      /* incident refractive index. */
...
void RecordR(double Refl,       /* reflectance. */
    InStru *In_Ptr, PhotonStru *Photon_Ptr, OutStru *Out_Ptr)
{
    double x = Photon_Ptr->x;
    double y = Photon_Ptr->y;
