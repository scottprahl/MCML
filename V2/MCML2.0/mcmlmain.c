...
time_t PunchTime(char F, char *Msg, InStru *In_Ptr)
{
    ...
    if (F == 1) {               /* show & pass real time. */
        ...
        sprintf(Msg, "Real time for this simulation: %.1f sec = %.2f hr.", real_time_secs, real_time_secs / 3600);
    }
...
void PredictDoneTime(long P1, InStru *In_Ptr)
{
    ...
    if (In_Ptr->control_bit == 1 || In_Ptr->control_bit == 3) {
        done_time = now + (time_t) ((PunchTime(2, "", In_Ptr) / (double) P1)
            * (In_Ptr->num_photons - P1));
        if (In_Ptr->control_bit == 3)
            done_time = (done_time < (now + In_Ptr->num_seconds - PunchTime(2, "", In_Ptr)))
...
void DoOneRun(short NumRunsLeft, InStru *In_Ptr, OutStru *Out_Ptr, char Type)
{
    ...
    do {
        ...
        if (In_Ptr->control_bit == 1)
            ...
            exit_switch = (i_photon > In_Ptr->num_photons) || (PunchTime(2, "", In_Ptr) >= In_Ptr->num_seconds);
    } while (!exit_switch);
