...
IsoList GetIsoValues(double Z_Min, double Z_Max)
{
    ...
    head = (IsoList) malloc((size_t) sizeof(struct Isolines));
    if (head == NULL)
        return (NULL);
...
PairList AllocPair(void)
{
    ...
    pair = (PairList) malloc((unsigned) sizeof(struct XYpairs));
    if (pair == NULL)
...
void GetAnIsoLine(IsoList IsoNode, double **Z, long IXmax, long IYmax, double Dx, double Dy)
{
    ...
    for (j = 0; j < IYmax - 1; j++)
        for (i = 0; i < IXmax - 2; i++)
            if (INTERSECTI(ival, Z, i, j) || INTERSECTJ(ival, Z, i, j)) {       /* found a pair. */
 ...
 ****/
char OutOfOrder(PairList This,  /* current pair. */
    PairList Next, struct XY Pmax)
{
    double x_this, y_this, x_next, y_next;
    short q0, q1;               /* Quadrants. */
    char out_order;

    if (This == NULL || Next == NULL)
        return (0);             /* This shouldn't happen. */
    x_this = This->x - Pmax.x;
    y_this = This->y - Pmax.y;
    q0 = GetQuadrant(x_this, y_this);
    x_next = Next->x - Pmax.x;
    y_next = Next->y - Pmax.y;
    q1 = GetQuadrant(x_next, y_next);
    ...
    if (q0 < q1)
        ...
        if (y_this * x_next < y_next * x_this)
            out_order = 0;
        else
            out_order = 1;
    }
...
{
    QUE qsave, qprint;          /* que to be printed. */
    QDATA p;                    /* QDATA is PairList. */
    char all_nulls;

    qprint.front = qprint.rear = NULL;
    qsave.front = qsave.rear = NULL;
    while (IsoHead != NULL) {
        fprintf(Isofile, "X%-8G\tY%-8G\t", IsoHead->iso_val, IsoHead->iso_val);
    ...
    do {
        ...
        while (!IsEmpty(qprint)) {
            Deque(&qprint, &p);
            if (p != NULL) {
                fprintf(Isofile, "%9.2E\t%9.2E\t", p->x, p->y);
...
void IsoPlot(double **Z,        /* the 2D array Z[i][j]. */
    long IXmax, long IYmax,     /* the 0<=i<=IXmax, 0<=j<=IYmax. */
    double Dx, double Dy)
{                               /* the gridline separations. */
    ...
    printf("The range of the value is %g to %g.\n", zmin, zmax);
